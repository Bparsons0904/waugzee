...
export function isDiscogsApiRequestData(
  obj: unknown,
): obj is DiscogsApiRequestData {
  if (!obj || typeof obj !== "object") return false;
...
export function isSyncErrorData(obj: unknown): obj is SyncErrorData {
  ...
  const data = obj as Record<string, unknown>;
  return typeof data.sessionId === "string" && typeof data.error === "string";
}
...
export class DiscogsProxyService {
   ...
   */
  async initiateCollectionSync(
    request: InitiateSyncRequest,
  ): Promise<SyncSession> {
    console.log("[DiscogsProxy] Initiating collection sync", request);

    try {
      const response = await api.post<SyncSession>(
        "/discogs/syncCollection",
        request,
      );
   ...
   */
  async updateDiscogsToken(
    token: string,
  ): Promise<{ message: string; tokenValid: boolean }> {
    console.log("[DiscogsProxy] Updating Discogs token");

    try {
      const response = await api.put<{ message: string; tokenValid: boolean }>(
        "/discogs/token",
        {
          discogsToken: token,
        },
   ...
   */
  async validateDiscogsToken(): Promise<{
    hasToken: boolean;
    tokenValid: boolean;
  }> {
  ...
  private setupMessageHandlers(): void {
    if (!this.webSocket) {
      console.error(
        "[DiscogsProxy] Cannot setup handlers - no WebSocket context",
      );
      return;
    }
    ...
    if (!this.webSocket.onSyncMessage) {
      console.warn(
        "[DiscogsProxy] WebSocket context does not support sync message handling",
      );
      return;
    }
  ...
  private handleIncomingMessage(message: WebSocketMessage): void {
    console.log(
      "[DiscogsProxy] Received WebSocket message",
      message.type,
      message.action,
    );
    ...
    try {
      switch (message.type) {
        ...
        default:
          console.log(
            "[DiscogsProxy] Ignoring unknown message type:",
            message.type,
          );
          break;
      }
  ...
  private async handleApiRequest(message: WebSocketMessage): Promise<void> {
    ...
    if (!isDiscogsApiRequestData(message.data)) {
      console.error(
        "[DiscogsProxy] Invalid API request data format",
        message.data,
      );
      return;
    }
    ...
    if (validationError) {
      console.error(
        "[DiscogsProxy] Request validation failed",
        validationError,
      );
      this.sendApiResponse(
        discogsApiService.createErrorResponse(requestId, validationError),
      );
      return;
    }
  ...
  private sendApiResponse(response: DiscogsApiResponse): void {
    if (!this.webSocket) {
      console.error(
        "[DiscogsProxy] Cannot send response - no WebSocket context",
      );
      return;
    }
  ...
  private handleSyncProgress(message: WebSocketMessage): void {
    ...
    if (!isSyncProgressData(message.data)) {
      console.warn(
        "[DiscogsProxy] Invalid sync progress data format",
        message.data,
      );
      return;
    }
    ...
    if (!this.isValidSyncProgress(progress)) {
      console.warn(
        "[DiscogsProxy] Progress data does not match SyncProgress interface",
        progress,
      );
      return;
    }
  ...
  private isValidSyncProgress(obj: unknown): obj is SyncProgress {
    ...
    return (
      ...
      (progress.estimatedTimeLeft === undefined ||
        typeof progress.estimatedTimeLeft === "string")
    );
  }
  ...
  private handleSyncComplete(message: WebSocketMessage): void {
    ...
    if (!isSyncCompleteData(message.data)) {
      console.warn(
        "[DiscogsProxy] Invalid sync complete data format",
        message.data,
      );
      return;
    }
  ...
  private handleSyncError(message: WebSocketMessage): void {
    ...
    if (!isSyncErrorData(message.data)) {
      console.warn(
        "[DiscogsProxy] Invalid sync error data format",
        message.data,
      );
      return;
    }
