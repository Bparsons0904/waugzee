...
func (ac *AdminController) SimplifiedProcess(ctx context.Context, req SimplifiedRequest) (*SimplifiedProcessResponse, error) {
	...
	// Map file types to their processing methods
	processingMethods := map[string]func(context.Context, string, string) (*services.ProcessingResult, error){
	...
	for _, fileType := range req.FileTypes {
		...
		result, err := method(ctx, filePath, processing.ID.String())
		if err != nil {
			fileResult.Success = false
